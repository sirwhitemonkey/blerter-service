...
if [ "$1" != "" ] && [ "$2" != "" ]; then
 ...
 case "$2" in
        ...
        export TOKEN_SERVICE_HOST=blerter-token.$1.svc.cluster.local
        export MASTER_SERVICE_HOST=blerter-master.$1.svc.cluster.local
        ...
        kubectl patch deployment -n $1 blerter-service -p \
                         '{"spec": {"template": {"spec":{"containers":[{"name": "blerter-service", "env":[{"name": "TOKEN_SERVICE_HOST", "value": '\"$TOKEN_SERVICE_HOST\"'},{"name": "MASTER_SERVICE_HOST", "value": '\"$MASTER_SERVICE_HOST\"'}]}]}}}}'
